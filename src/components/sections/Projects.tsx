                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <div className="flex flex-wrap gap-2 mb-5">
                    {project.stack.map((tech) => (
                      <span key={tech} className="tag-mono">{tech}</span>
                    ))}
                  </div>
                    ...
                    <div className="flex flex-wrap gap-4 mb-5 font-mono-tech text-xs text-text-secondary">
                      {project.stats.map((stat) => (
                        <span key={stat.label} className="flex items-center gap-1.5">
                          {stat.label.includes('star') ? <Star size={11} className="text-accent" /> : null}
                          {stat.label.includes('download') ? <Download size={11} className="text-accent" /> : null}
                  ...
                  <div className="flex gap-3 mt-auto pt-4 border-t border-border-muted">
                    {project.links.github && (
                      <a href={project.links.github} target="_blank" rel="noopener noreferrer" className="btn-ghost text-xs py-2 px-3">
                        <GithubIcon style={{ width: 13, height: 13 }} />
                        GitHub
                      </a>
                    )}
                    {project.links.live && (
                      <a href={project.links.live} target="_blank" rel="noopener noreferrer" className="btn-ghost text-xs py-2 px-3">
